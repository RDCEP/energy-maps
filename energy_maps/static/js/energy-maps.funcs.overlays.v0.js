...
EnergyMaps = (function (energy_maps, InfrastructureSet) {
    ...
    (ctx, queued_data)
  {
    ...
    ctx.lineWidth = energy_maps.state_boundaries.width / TRANSFORM.k;
    // ctx.setLineDash([
    //   0,
    ...
    (ctx, queued_data)
  {
    ...
    for (let i = 0; i < bands_length; ++i) {
      ...
      if (output_geojson[i] === undefined || energy_maps.k_changed) {
      ...
      ctx.beginPath();
      energy_maps.path(output_geojson[i]);
      ctx.fill();
    }
    energy_maps.finish_loading_layer();
  };
  ...
  const state_boundaries = new StateBoundary(
    ...
    'layers',
    [{
      draw_layer: _draw_state_boundaries,
